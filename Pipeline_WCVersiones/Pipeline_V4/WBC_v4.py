...
class WBC29:  # clase del código
    ...
    try:
        ...
        error_403 = driver.execute_script("return document.body.innerText;")
        if "Forbidden" in error_403:
            Log().error(f"Error 403: You don't have permission to access /{nueva_version}/dbupdate.ashx on this server.")
            Log().info("=" * 70 + "\n")
        else:
            Log().info("No se detectó bloqueo de acceso, se continúa con la validación.")
            WebDriverWait(driver, 15).until_not(EC.text_to_be_present_in_element(
                (By.TAG_NAME, "body"), "403"))
            mensaje_actualizacion = driver.execute_script("return document.body.innerText;")
            if "Finalizado" in mensaje_actualizacion:
                Log().info(" Actualización de la base de datos, finalizada con éxito")
            else:
                Log().warning(" No se encontró un mensaje de Finalizado.")

            try:
                error_count_element = WebDriverWait(driver, 20).until(EC.visibility_of_element_located((
                    By.XPATH, "//onresults/error-count")))
                error_text = error_count_element.text  # Obtenemos el texto del elemento xpath
        ...
        for elem in version_actual:
            ...
            if match:
                ...
                try:
                    ultima_version = max(todas_las_versiones, key=lambda v: [int(x) for x in v.split(".")])
                    element = driver.find_element(By.XPATH,
                                                  "//option[@value='5.0.0.50']/preceding-sibling::option[5]")
                    element.click()
                    Log().info(f" Nueva versión creada: {nueva_version}")
