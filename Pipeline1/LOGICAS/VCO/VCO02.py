...
def ingreso(self):
    ...
    try:
        ...
        Log().error(f" El ingreso al login no fue correcto, favor de validar el siguiente error: {e}")
        raise
...
def validar_articulos(self):
    try:
        ...
        Log().error(f"No se pudo ejecutar la función ingreso_tabla_artículos {e}")
        raise
    try:
        ...
        Log().info(" Ingresa el código del articulo ")
        time.sleep(1)

        refresca = wait.until(EC.presence_of_element_located((By.XPATH, Configuracion.btn_refresca)))
        refresca.click()
        Log().info(" Se presiona el botón 'Refrescar', para mostrar la información filtrada.")
        time.sleep(2)
        ...
        time.sleep(2)
        Log().info(" Se presiona el botón 'Cerrar', para cerrar la ventana")

    except Exception as e:  # pragma: no cover
        Log().error(f"No se validaron los artículos, favor de validar el error: {e}")   # pragma: no cover
        raise
...
def validacion_precios():
    try:
        ...
        Log().error(f" El ingreso para a la pantalla de precios no fue correcto, favor de validar el error: {e}")
        time.sleep(2)
        return False
    ...
    try:
        ...
        Log().error(f"No se pudo ingresar al filtro, favor de validar el error: {e}")
        time.sleep(2)
        return False

    try:
        # Ingreso de articulo
        codigo_articulo = wait.until(EC.presence_of_element_located((By.XPATH, Configuracion.campo_codart)))
        codigo_articulo.send_keys(Configuracion.codigo_art6)
        ...
        codigo_lista_precios = wait.until(EC.presence_of_element_located((By.XPATH, Configuracion.campo_codlp)))
        codigo_lista_precios.click()
        codigo_lista_precios.send_keys(Configuracion.lista_precio)
        ...
        ordenar_fecha = wait.until(EC.presence_of_element_located((By.XPATH, Configuracion.columna_fecha)))
        action \
            .click(ordenar_fecha) \
            .pause(0) \
            .double_click(ordenar_fecha) \
            .pause(0) \
            .release()
        action.perform()
        ...
        cierra_ventana = wait.until(EC.presence_of_element_located((By.XPATH, Configuracion.btn_cerrar_ventana)))
        cierra_ventana.click()
        Log().info(" Se presiona el boton 'Cerrar', para cerrar la ventana 1")
    except Exception as e:  # pragma: no cover
        Log().error(f"No se pudieron validar los precios, favor de validar el error: {e}")  # pragma: no cover
        raise
...
def validacion_dz1(self):
    try:
        ...
        Log().error(f"No se pudo ejecutar la funcion ingresareportes_dz {e}")
        raise
    try:
        ...
        stockinicial = df_stock[(df_stock['Codigo'] == Configuracion.codigo_art6)]
...
def validacion_dz2():
    ...
    try:
        ...
        # Imprime el stock
        Log().info(f"El stock de los articulos despues de la venta de contado es: {stockinicial}")
...
def datos_vco(self):
    try:
        ...
        Log().error(f"No se pudo ejecutar la funcion venta_contado {e}")
        raise
    try:
        ...
        articulo.send_keys(Configuracion.codigo_art6)
    ...
    try:
        cantidad = wait.until(EC.presence_of_element_located((By.XPATH, Configuracion.campo_cantidad)))
        time.sleep(1)
        cantidad.click()
        time.sleep(1)
        cantidad.send_keys(Configuracion.cantidad_art1)
...
def impuesto_venta(self):
    try:
        ...
        valida_iva = wait.until(EC.presence_of_element_located((By.XPATH, Configuracion. iva_venta_contado2))).text
        self.assertEqual("68.94", valida_iva, "El IVA es correcto")
        Log().info("El IVA de la venta es correcto")
        time.sleep(1)

        valida_total = wait.until(EC.presence_of_element_located((By.XPATH, Configuracion.total_venta_contado2))).text
        self.assertEqual("499.84", valida_total, "El total es correcto")
...
def documento_emitido(self):
    try:
        ...
        Log().error(f"No se pudo ejecutar la funcion documentos_emitidos {e}")
        raise
    try:
...
class Test(unittest.TestCase):
    ...
    def test_007(self):
        """Validar precios en el artículo"""
